...
int existing_fruit(std::shared_ptr<ElementoLista> const &fruit, std::vector<Pedido> const &v)
{
    ...
    for (auto elem : v)
    {
        if (elem.fruta == fruit->getPedido().fruta)
            return pos;
        pos++;
    }
    return -1;
...
Lista Lista::unificar(void) const
{
    ...
    while (iter != nullptr)
    {
        if (existing_fruit(iter, vec) == -1)
            vec.push_back(iter->getPedido());
        else
            vec[existing_fruit(iter, vec)].cantidad += iter->getPedido().cantidad;
        iter = iter->next;
    }
    for (auto elem : vec)
    {
